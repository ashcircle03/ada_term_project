# %% [markdown]
# # 07 · 신규 셀러 프로파일링 PoC — 위시리스트로 '유사 + 성공' 셀러 레퍼런스
#
# **목적.** 빈티지 1점물은 동일상품 시세·매칭이 불가능하다(당근·KREAM식 표준화 공식 적용 불가).
# 대신 FF만의 자산인 **공개 위시리스트(수요 측 취향 그래프)** 로, 셀링 이력이 0인 **신규 진입 셀러**를
# 위시리스트만으로 프로파일링해 "취향이 비슷하면서 잘 파는 기존 셀러"를 레퍼런스로 제시할 수 있는지
...
# 부재 시 사용할 후보 프로파일링 신호가 될 수 있다.
#
# **무엇을 보이고 무엇을 안 보이나(정직).** (i) 수요 취향이 또렷한가, (ii) 취향 이웃으로 프로파일을
# 넓힐 수 있는가, (iii) **위시로 검색한 유사 셀러가 실제 그 사람의 셀링 취향과도 가까운가(leave-one-out,
# 무작위 대비)**, (iv) 신규 셀러마다 '유사+성공' 레퍼런스가 실제로 산출되는가(도달률) — 즉 **메커니즘
# feasibility** 까지다. 단면 자료라 '진짜 신규 셀러의 위시→이후 판매'를 종단으로 본 건 아니며(기존
...
# %% [markdown]
# ## 2. 브랜드 친화도 (co-wishlist) — 정확 브랜드를 넘어 '취향 이웃'으로
#
# 같은 사용자가 함께 찜하는 브랜드는 취향이 가깝다. 브랜드 동시-위시 행렬에서 코사인 유사도를 구하면,
# 특정 브랜드의 *취향 이웃 브랜드*를 얻어 신규 셀러의 취향 프로파일을 (키워드가 아니라) 취향으로 넓힐 수 있다.

# %%
brands = list(wlm["brand"].value_counts().head(300).index)
bidx = {b: i for i, b in enumerate(brands)}
wm = wlm[wlm.brand.isin(brands)].copy()
ofac = wm["owner_seller_id"].astype("category")
M = csr_matrix((np.ones(len(wm)), (ofac.cat.codes.values, wm["brand"].map(bidx).values)),
               shape=(ofac.cat.categories.size, len(brands)))
M.data[:] = 1.0                                   # binary(찜 여부)
Xb = normalize(M.T.tocsr())                       # brand × owner, L2 정규화
sim_b = (Xb @ Xb.T).toarray()                     # 브랜드-브랜드 코사인(동시-위시 기반)
def neighbors(b, k=5):
    i = bidx[b]; s = sim_b[i].copy(); s[i] = -1
    return [(brands[j], round(float(s[j]), 2)) for j in np.argsort(s)[::-1][:k]]
for b in ["Stone Island", "Supreme", "Maison Margiela"]:
    if b in bidx:
        print(f"  {b} 의 취향 이웃: {neighbors(b)}")

# %% [markdown]
# ## 3. 콜드스타트 PoC — 위시로 찾은 유사 셀러가 실제 셀링과도 가까운가 (leave-one-out)
#
...
# %% [markdown]
# ## 4. 워크드 예시 — 신규 셀러의 위시 프로파일 → 레퍼런스 카드 + comp 가격 밴드
#
# 한 신규-시뮬 셀러를 골라, (a) 위시로 검색된 상위 유사 셀러(대표 브랜드·전환율·중앙 가격)와
# (b) 그가 올리려는 상품의 **comp 가격 밴드**(당근식 동일상품이 없으니 동종 유사 comp로 산출)를
# 레퍼런스 카드처럼 보여준다.

# %%
sel_meta = sel.set_index("seller_id")
rng2 = np.random.RandomState(7)
cand = [i for i in range(len(both)) if w_tot[both[i]] >= 10]
ex = rng2.choice(cand)
ex_sid = both[ex]
sims_ex = (W[ex] @ ST).toarray().ravel(); sims_ex[both_in_est[ex]] = -1
top5 = np.argsort(-sims_ex)[:5]
wish_top = wlm[wlm.owner_seller_id == ex_sid]["brand"].value_counts().head(4)
print(f"[신규 셀러 시뮬] 위시 상위 브랜드: {list(wish_top.index)}  (셀링은 가린 상태)")
print("─ 추천 레퍼런스(위시로 검색된 '취향이 비슷한' 셀러) ─")
for j in top5:
    sid = est[j]
    tb = ", ".join(lst[lst.seller_id == sid]["brand"].value_counts().head(3).index.astype(str))
    st = sel_meta.loc[sid, "sell_through"]; mp = sel_meta.loc[sid, "median_price"]
    flag = "★성공" if st >= hi_bar else ""
    print(f"   유사도 {sims_ex[j]:.2f} | 전환 {st:.0%} {flag} | 대표 {tb} | 중앙가 {mp:,.0f}원")

# comp 가격 밴드 — 신규 셀러의 top 위시 브랜드 × 대표 카테고리의 동종 가격대(=유사 셀러들이 파는 가격)
top_brand = wish_top.index[0]
bl = lst[(lst.brand == top_brand) & lst.category_l1.notna()]
EX_BAND = {}
if len(bl):
    focal_cat = bl["category_l1"].value_counts().index[0]
    band = lst[(lst.brand == top_brand) & (lst.category_l1 == focal_cat)]["price_final"].dropna()
    p25, med, p75 = band.quantile([.25, .5, .75])
    print(f"─ comp 가격 밴드(동일상품 대신 동종 유사 comp) ─")
    print(f"   {top_brand} · {focal_cat} (n={len(band)}): p25–p75 = {p25:,.0f}–{p75:,.0f}원 (중앙 {med:,.0f})")
    EX_BAND = {"brand": str(top_brand), "category": str(focal_cat), "n": int(len(band)),
               "p25": int(p25), "median": int(med), "p75": int(p75)}
...
res = {
    ...
    "brand_neighbors_example": {b: neighbors(b) for b in ["Stone Island", "Supreme"] if b in brands},
    "onboarding_poc": {
        "k_pool": K,
