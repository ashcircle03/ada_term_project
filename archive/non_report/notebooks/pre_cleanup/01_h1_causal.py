...
# *미관측* 교란은 잡지 못하므로, 그 취약성은 06 노트북에서 **E-value**(추정 효과를
...
# (4) PSM 처치효과(ATT). 산출 수치는 results/h1.json.
...
#   역선택과 '오래 걸어 둔 매물이 더 팔리는' 절단 — 곧 노력↔판매의 음상관을 만들 수 있는 *관측 가능한*
#   교란이라 통제에 넣는다.
# - **처치 정의.** PSM 처치 `compliant = 사진≥3 & 설명≥150자`는 임의 기준이 아니라 **플랫폼 등록 가이드의
#   권고 임계값** 그대로다(가이드 준수 = 처치).
# - **caliper 0.02 · 표준화.** 성향점수(로짓) 표준편차의 ~0.2배 관례로 너무 다른 쌍의 매칭을 막는다. 표현
#   변수는 z-표준화해 계수를 'SD 1 증가당 OR'로 해석한다.
...
# 공변량 균형 점검과 E-value 민감도는 06 노트북에서 수행한다.
