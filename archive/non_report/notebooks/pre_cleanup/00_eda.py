...
#
# 본 분석에서 사용한 변수와 그 전처리·선택 이유를 정리한다(교수 검증용). 구체 구현은
...
# | `category_l1`, `category_l2` | 남/여, 하위 카테고리 상위 20 + 기타 | 매물 종류라는 구조 요인 통제 |
...
# | `view_count`, `like_count` | 예측에서 제외 | 등록 이후 누적되는 사후 변수(누수). 진단·수요 신호로만 사용 |
...
# 라벨 순열검정으로·두 모형 ΔAUC=0을 부트스트랩 신뢰구간으로, (H3) 군집 구조 부재를 무작위
# 참조 실루엣 분포로·아키타입 간 전환율 동일을 크러스컬–월리스 검정과 효과크기 ε²로 검정한다.
...
(by_yr["unsold"]*100).plot(kind="bar", ax=ax, color="#c77", rot=0)
ax.axhline((1-cdf.is_sold.mean())*100, ls="--", c="gray", lw=1, label="전체 평균")
ax.set(title="등록 코호트별 미판매율 (표집·절단과 무관함 확인)", ylabel="미판매 %", xlabel="등록 연도"); ax.legend()
fig.tight_layout(); fig.savefig(FIG / "eda_cohort.png", bbox_inches="tight"); plt.show()
